...
class MiniZincEnvironment:
    """Handles the symbolic derivation of MiniZinc Code via EBNF."""
    ...
    def _get_leftmost_nt(self, state: Tuple[str, ...]) -> int:
        for i, symbol in enumerate(state):
            if symbol.startswith("<") and symbol.endswith(">"):
                return i
        return -1

    def get_valid_actions(self, state: Tuple[str, ...]) -> List[Tuple[str, ...]]:
        idx = self._get_leftmost_nt(state)
        if idx == -1: return[]
        nt = state[idx]
        return [tuple(prod) for prod in self.grammar[nt]]

    def apply_action(self, state: Tuple[str, ...], action: Tuple[str, ...]) -> Tuple[str, ...]:
        idx = self._get_leftmost_nt(state)
    ...
    def compute_reward(self, state: Tuple[str, ...]) -> float:
        """Evaluates how well the generated code matches the NL prompt semantics."""
        code = "".join(state)
        reward = 0.0
        
        # Simple string-matching mock evaluator (Replace with an actual MiniZinc compiler check)
        if "greater than 5" in self.target_prompt.lower() and "integer x" in self.target_prompt.lower():
            if "var int: x;" in code: reward += 0.3
            if "constraint x > 5;" in code: reward += 0.5
            if "solve satisfy;" in code: reward += 0.2
